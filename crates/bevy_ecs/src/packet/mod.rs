...
pub(crate) struct PacketInSystem<E: SystemInput> {
    pub v: BoxedSystem<E, ()>,
    pub tid: TypeId,
}
pub(crate) struct RegisteredSystems<E: SystemInput>{
...
pub fn run_this_packet_system<E>(event: E, world: &mut World)
where 
    E: Packet,
    ...
    E: for<'e> SystemInput<Inner<'e> = E>
{
    world.with_packet_system::<E>(|world, systems| {
        ...
        system.v.run(event, world).unwrap();
